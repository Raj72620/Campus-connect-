...
const originChecker = (origin, callback) => {
  ...
  // Allow Netlify URL specifically from user request
  if (origin === 'https://campuss-connect.netlify.app') return callback(null, true);
...
// Handle API 404s separately to avoid returning HTML
app.use('/api/*', (req, res) => {
